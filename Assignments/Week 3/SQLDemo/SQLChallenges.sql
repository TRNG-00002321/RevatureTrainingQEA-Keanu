-- Active: 1763664936057@@127.0.0.1@3306@Chinook
-- Parking Lot*******
-- *                *
-- *                *
--- *****************
...
SELECT InvoiceDate, COUNT(*) AS Total_Invoices FROM `Invoice` WHERE `InvoiceDate` LIKE '2021%'
...
SELECT `AlbumId`,`Title`,`Name` FROM `Album` INNER JOIN `Artist` ON Album.`ArtistId` = Artist.`ArtistId` 

-- (inner keyword is optional for inner join)
-- All songs of the rock genre
SELECT Track.Name AS Song , Genre.`Name` AS Genre 
FROM Track 
JOIN `Genre` ON Track.`GenreId` = Genre.`GenreId` 
WHERE Genre.`Name` LIKE 'Rock'

-- Show all invoices of customers from brazil (mailing address not billing)
SELECT `Invoice`.`Total` AS Invoice, CONCAT(`Customer`.`FirstName`," ",`Customer`.`LastName`) As Customer
FROM `Invoice`
JOIN `Customer` ON Customer.`Country` = `Invoice`.`BillingCountry`
WHERE Customer.`Country` LIKE 'Brazil'
