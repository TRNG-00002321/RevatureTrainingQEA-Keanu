...
class TestBasicRequest:
    ...
    def test_single_post(self, base_url, session):
    ...
    def test_single_post_params(self, base_url, session):
        params = {"ID": 1}
        response = session.get(f"{base_url}/posts/", params=params)
        assert response.status_code == 200

    def test_create_post(self, base_url, session):
    ...
    def test_user_names(self, base_url, session, user_id, expected_name):
        """Test usernames match expected values"""
